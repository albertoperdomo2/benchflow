...
def _hostpath_nvme_metric_rows(paths: ArtifactPaths, metric_name: str) -> list[dict]:
    rows = _metric_rows(paths, metric_name)
    return [
        row
        for row in rows
        if (row.get("labels") or {}).get("mountpoint")
        and (row.get("labels") or {}).get("device")
    ]
...
def _build_system_figures(
    ...
    storage_nvme_bytes_per_second_by_node_device = _rows_to_grouped_series(
        _hostpath_nvme_metric_rows(
            paths, "storage_nvme_bytes_per_second_by_node_device"
        ),
        _node_device_direction_label,
    )
    storage_nvme_iops_by_node_device = _rows_to_grouped_series(
        _hostpath_nvme_metric_rows(paths, "storage_nvme_iops_by_node_device"),
        _node_device_direction_label,
    )
    storage_nvme_busy_seconds_per_second_by_node_device = _rows_to_grouped_series(
        _hostpath_nvme_metric_rows(
            paths, "storage_nvme_busy_seconds_per_second_by_node_device"
        ),
        _node_device_label,
    )
    storage_nvme_filesystem_avail_bytes_by_node_mount = _rows_to_grouped_series(
        _hostpath_nvme_metric_rows(
            paths, "storage_nvme_filesystem_avail_bytes_by_node_mount"
        ),
        _node_mount_label,
    )
    storage_nvme_filesystem_usage_percent_by_node_mount = _rows_to_grouped_series(
        _hostpath_nvme_metric_rows(
            paths, "storage_nvme_filesystem_usage_percent_by_node_mount"
        ),
        _node_mount_label,
    )
        ...
        if timestamps:
            figures.append(
                _build_grouped_line_figure(
                    ...
                    segments=segments,
                    title="(ap) Node NFS request rate",
                    subtitle=(
        ...
        if timestamps:
            figures.append(
                _build_grouped_line_figure(
                    ...
                    segments=segments,
                    title="(aq) Node NFS retransmissions",
                    subtitle=(
