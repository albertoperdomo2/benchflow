...
def render_rhoai_release_gateway_listener(
    ...
    """Create an isolated listener on the externally published Gateway.

    The bootstrap Gateway owns the external hostname and load balancer. A
    release-specific listener section keeps its HTTPRoute separate while all
    releases remain reachable through that single published endpoint.
    """
    listener = copy.deepcopy(config.listener)
    listener["name"] = rhoai_release_gateway_listener_name(plan)
    # Reusing the hostname on port 443 would conflict with the bootstrap
    # listener. sectionName is the routing isolation boundary here.
