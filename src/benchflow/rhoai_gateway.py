...
def rhoai_release_gateway_name(plan: ResolvedRunPlan) -> str:
    """Return a cluster-unique Gateway name for a BenchFlow release."""
    ...
    release = "".join(
        ...
        for character in plan.deployment.release_name.lower()
    ).strip("-")
    return f"benchflow-{release[:40]}-{suffix}-gateway"
